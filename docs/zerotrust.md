...
| Network egress | optional `gh` PR check, release-freshness check, webhook notifier available | all disabled -- release-freshness check and webhook notifier via `network.egress_allowed: false` (wins regardless of the feature's own toggle); `gh` PR check via its own `telemetry.gh_pr_check` flag |
...
Under the zerotrust preset, SessionStart auto-runs this check. A missing, modified, or **extra** (undeclared) file under the tracked globs counts as a mismatch: an added `lib/*.py` would shadow a real import via `PYTHONPATH`, so addition is treated as tampering too. On mismatch, every hook in the session exits silently after writing `~/.claude/presence/.integrity-blocked`, and the failure is recorded in the audit log. The next clean SessionStart removes the marker.

Because the plugin directory is integrity-checked, do not drop custom presets into the plugin's `presets/` directory under zerotrust; that would fail the gate. Custom presets belong in `~/.claude/presence/presets/<name>.json`, which `_load_preset` reads first and which sits outside the manifest scope.
